...
Table.prototype.isValidValueForProperty = function(property, value) {
  if (this._schema == null) {
    return true;
  }
  const propSchema = this._schema[property];
  return propSchema.validate(value);
};

Table.prototype.isValidProperty = function(property) {
  return this._schema == null || this._schema[property] != null;
};
...
Table.prototype.getSchemaProperty = function(property) {
  if (this._schema == null) {
    return null;
  }
  if (this._schema[property] == null) {
    throw "Property specified does not exist in schema.";
  }
  return this._schema[property];
};
...
Table.prototype.addGlobalIndex = function(name, primary, sort) {
  const index = new Index(name).primary(primary);
  if (sort) {
    index.sort(sort);
  }
...
Table.prototype.addLocalIndex = function(name, primary, sort) {
  const index = new Index(name).primary(primary).local();
  if (sort) {
    index.sort(sort);
  }
  this._indexes.push(index);
...
Table.prototype.exists = async function() {
  ...
  try {
    const described = await internals.describe(this._name);
    if (described) {
...
Table.prototype.create = async function() {
  ...
  for (const index of indexes) {
    const indexArrayName = index.getLocal() ? "LocalSecondaryIndexes" : "GlobalSecondaryIndexes";
    const idxPrimary = index.getPrimary(),
      idxSort = index.getSort();
    const indexParams = {
    ...
    internals.addKeyAttribute(
      params,
      idxPrimary,
      "HASH",
      utils.schemaToDynamo(schema[idxPrimary]),
      indexParams
    );
    if (idxSort != null) {
      internals.addKeyAttribute(
        ...
        idxSort,
        "RANGE",
        utils.schemaToDynamo(schema[idxSort]),
        indexParams
      );
    }
