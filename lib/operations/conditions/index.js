...
function addProperty(condition, propName) {
  if (propName.indexOf(".") === -1) {
    condition.nameField = `#${utils.fname(condition)}`;
    condition.cndNames = {
    ...
    for (const nest of nestedProps) {
      const fname = `#${utils.fname(condition)}`;
...
Condition.prototype.property = function property(propName) {
  ...
  return addProperty(this, propName);
};

Condition.prototype.size = function size(propName) {
  utils.checkNextFunc(this, size.name);
  utils.setNextFunc(this, utils.NextFunction.NUMERIC);
  return addProperty(this, propName);
};
